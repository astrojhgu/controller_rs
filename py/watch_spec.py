...
p.open_live(dev_name, 1600, True , 100)
...
spec_data=np.array([[0. for i in range(0,2048)] for i in range(len(mac_idx_map)*8)])
fft_data=np.array([[0.+0.0j for i in range(0,2048)] for i in range(len(mac_idx_map)*8)])
corr_data=np.array([[0.+0.0j for i in range(0,2048)] for i in range(len(mac_idx_map)*8)])
...
try:
    while True:
        pkt=next(p)
        if pkt is None:
            print("pkt missed, send request again")
            request.wait()
            request=subprocess.Popen(["../target/release/fetch_fft", "enp3s0", "../param1.yaml"])
            continue
        ...
        if src_mac in mac_idx_map and pktlen==1024+12+2+4+1:
            ...
            fft_data1=1.j*fft_data1[::2]+fft_data1[1::2]
            ...
            #spec_data[port_id,ch1:ch2]=spec_data[port_id, ch1:ch2]*update_ratio+(1.-update_ratio)*ps
            spec_data[port_id,ch1:ch2]+=ps
            fft_data[port_id, ch1:ch2]=fft_data1
