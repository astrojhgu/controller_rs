...
impl AdcMsg{
    ...
    pub fn get_raw_data(&self)->Vec<u8>{
        ...
        match self{
            ...
            &AdcMsg::Cfg {ref io_delay, packet_gap, counter_sync, counter_wait, trig_out_delay, optical_delay}=>{
                let mut result=Vec::new();
            ...
            &AdcMsg::FftParam {fft_shift, truncation}=>{
                vec![(fft_shift&0x00ff) as u8,
                     (fft_shift>>8) as u8,
            ...
            &AdcMsg::PhaseFactor {ref value}=>{
                let data=value.chunks(2).fold(Vec::<u16>::new(), |mut a,b|{a.push(b[1]);a.push(b[0]);a}).into_boxed_slice();
                let cap=data.len()*2;
