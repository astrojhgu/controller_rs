...
impl AdcMsg {
    pub fn from_yaml(cfg: &Value) -> Option<AdcMsg> {
        ...
        match typecode {
            ...
            Some("Ctrl") => match load_ctrl_param(cfg, "param") {
            ...
            Some("FftParam") => {
                ...
                Some(AdcMsg::FftParam {
                    fft_shift,
                    truncation,
                })
            }
            Some("PhaseFactor") => match cfg["value"].as_sequence() {
                Some(vv) => {
            ...
            Some("XGbeId") => match cfg["value"].as_sequence() {
                Some(v) => Some(AdcMsg::XGbeId {
                    value: v.iter().map(|v| v.as_u64().unwrap() as u8).collect(),
                }),
                _ => None,
            },
            _ => None,
