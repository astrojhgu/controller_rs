...
pub fn send_udp_buffer(cap:&mut Capture<Active>, buf:&[u8], dst_mac:[u8;6], src_mac:[u8;6],
                         dst_ip:[u8;4], src_ip:[u8;4],
                         dst_port:u16, src_port:u16,mtu_len:usize)->Result<(), Error>{
    if buf.is_empty(){
        let header=compose_udp_header(&dst_mac,&src_mac, &dst_ip, &src_ip, dst_port, src_port,0);
        ...
        for x in buf.chunks(mtu_len){
            let header=compose_udp_header(&dst_mac,&src_mac, &dst_ip, &src_ip, dst_port, src_port,x.len() as u16);
            let mut sub_buf=Vec::new();
            sub_buf.extend_from_slice(&header[..]);
